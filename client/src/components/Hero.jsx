...
const Hero = () => {
  ...
  // 1. GET TODAY'S DATE IN THE CORRECT FORMAT
      ...
      onSubmit={handleSearch} className='flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-1g md:rounded-full
        ...
        gap-10 min-md:ml-8'>
          ...
          <div className='flex flex-col items-start gap-2'>
            ...
            {/* 2. CORRECTED MINIMUM DATE FOR PICKUP */}
            <input value={pickupDate} onChange={e=>setPickupDate(e.target.value)} type="date" id="pickup-date" min={today} className='text-sm text-gray-500' required />
          </div>
          <div className='flex flex-col items-start gap-2'>
            <label htmlFor="return-date">Return Date</label>
            {/* 3. ADDED MINIMUM DATE FOR RETURN (LINKED TO PICKUP DATE) */}
            <input value={returnDate} onChange={e=>setReturnDate(e.target.value)}
              type="date" id="return-date" min={pickupDate || today} className='text-sm text-gray-500' required />
          </div>
